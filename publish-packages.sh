...
aws s3 sync "${source_base_uri}/deb" "${destination_base_uri}" --acl public-read
aws s3 sync "${source_base_uri}/rpm" "${destination_base_uri}" --acl public-read
...
aws s3 cp "${source_base_uri}/goreleaser" "${destination_base_uri}" --acl public-read --recursive --exclude "*" --include "*.tar.gz" --include "*.zip"

# Sync goreleaser dirs for non-standard builds
other_builds=("cgo-darwin-10.15" "fips-openssl-1.0" "fips-openssl-1.1")
...
aws s3 cp "${source_base_uri}/goreleaser" "${checksums_dir}" --recursive --exclude "*" --include "*.txt"
